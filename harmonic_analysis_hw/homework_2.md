...
$$
\begin{aligned}
&\ \bigg(\int_0^\infty\lambda^{q-q_0-1}\bigg(\int_X|f_0|^{p_0}\ d\mu\bigg)^{\frac{q_0}{p_0}}\ d\lambda\bigg)^{\frac{p_0}{q_0}}\\
=&\ \bigg(\int_0^\infty\bigg(\lambda^{\frac{(q-q_0-1)p_0}{q_0}}\int_X|f_0|^{p_0}\ d\mu\bigg)^{\frac{q_0}{p_0}}\ d\lambda\bigg)^{\frac{p_0}{q_0}}\\
=&\ \sup_{\begin{aligned}g(\lambda)\in L^{1/(1-p_0/q_0)}\\\|g\|_{1/(1-p_0/q_0)}=1\end{aligned}}\bigg\{\bigg|\int_0^\infty g(\lambda)\cdot\lambda^{\frac{(q-q_0-1)p_0}{q_0}}\int_X|f_0|^{p_0}\ d\mu\ d\lambda\bigg|\bigg\}\\
=&\ \sup_{\begin{aligned}g(\lambda)\in L^{1/(1-p_0/q_0)}\\\|g\|_{1/(1-p_0/q_0)}=1\end{aligned}}\bigg\{\bigg|\int_0^\infty g(\lambda)\cdot\lambda^{\frac{(q-q_0-1)p_0}{q_0}}\int_{\{x:|f(x)|>C\lambda\}}|f|^{p_0}\ d\mu\ d\lambda\bigg|\bigg\}\\
=&\ \sup_{\begin{aligned}g(\lambda)\in L^{1/(1-p_0/q_0)}\\\|g\|_{1/(1-p_0/q_0)}=1\end{aligned}}\bigg\{\bigg|\int_X|f|^{p_0}\int_0^{|f(x)|/C} g(\lambda)\cdot\lambda^{\frac{(q-q_0-1)p_0}{q_0}}\ d\mu\ d\lambda\bigg|\bigg\}\\
\leq& \ \sup_{\begin{aligned}g(\lambda)\in L^{1/(1-p_0/q_0)}\\\|g\|_{1/(1-p_0/q_0)}=1\end{aligned}}\bigg\{\bigg|\int_X|f|^{p_0}\bigg(\int_0^{|f(x)|/C} |g(\lambda)|^{1/(1-p_0/q_0)}\ d\lambda\bigg)^{1-\frac{p_0}{q_0}}\\&\cdot \bigg(\int_0^{|f(x)|/C} \lambda^{q-q_0-1}\ d\lambda\bigg)^{\frac{p_0}{q_0}}\ d\mu\bigg|\bigg\}\\
\leq&\ \bigg|\int_X|f|^{p_0}\cdot \bigg(\int_0^{|f(x)|/C} \lambda^{q-q_0-1}\ d\lambda\bigg)^{\frac{p_0}{q_0}}\ d\mu\bigg|\\
=&\ \bigg|\bigg(\frac{1}{(q-q_0)C^{q-q_0}}\bigg)^{\frac{p_0}{q_0}}\cdot\int_X|f(x)|^{\frac{p_0\cdot q}{q_0}}\ d\mu\bigg|\\
=&\ \bigg|\bigg(\frac{1}{(q-q_0)C^{q-q_0}}\bigg)^{\frac{p_0}{q_0}}\cdot\int_X|f(x)|^{p}\ d\mu\bigg|\\
=&\ \bigg|\bigg(\frac{1}{(q-q_0)C^{q-q_0}}\bigg)^{\frac{p_0}{q_0}}\bigg|\cdot\|f\|_p^p\\
\end{aligned}
$$
where the second last equality comes from the condition concerning $\theta$.
Consequently, we can bound $\|Tf\|_q^q$ using $\|f\|_p^q$:
